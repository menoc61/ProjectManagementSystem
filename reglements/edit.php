...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (mysqli_query($conn, $sql)) {
        $success = "Règlement modifié avec succès";
    } else {
        $error = "Erreur lors de la modification du règlement: " . mysqli_error($conn);
    }
}

$sql = "SELECT * FROM REGLEMENT WHERE IdReglement = $id";
...
<head>
    ...
    <title>Modifier un Règlement - Gestion de Projets</title>
        ...
        <div class="row">
            ...
            <div class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2">Modifier un Règlement</h1>
                    <div class="btn-toolbar mb-2 mb-md-0">
                        ...
                        <a href="view.php?id=<?php echo $id; ?>" class="btn btn-sm btn-info">
                            <i class="bi bi-eye"></i> Voir le règlement
                        </a>
                    </div>
                </div>
                ...
                <div class="alert alert-success" role="alert">
                    <?php echo $success; ?>
                    <a href="view.php?id=<?php echo $id; ?>" class="alert-link">Voir le règlement</a> ou 
                        ...
                        <form method="post" action="edit.php?id=<?php echo $id; ?>">
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="idClient" name="idClient" required>
                                    ...
                                    ?>
                                    <option value="<?php echo $client['IdClient']; ?>" <?php echo $reglement['IdClient'] == $client['IdClient'] ? 'selected' : ''; ?>>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="date" class="form-control" id="dateReglement" name="dateReglement" value="<?php echo $reglement['DateReglement']; ?>" required>
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label for="heureReglement" class="form-label">Heure du règlement</label>
                                    <input type="time" class="form-control" id="heureReglement" name="heureReglement" value="<?php echo date('H:i', strtotime($reglement['HeureReglement'])); ?>">
                                </div>
                            </div>
                            
                            <div class="mb-3">
                                <label for="montantReglement" class="form-label">Montant (FCFA) *</label>
                                <input type="number" step="0.01" class="form-control" id="montantReglement" name="montantReglement" value="<?php echo $reglement['MontantReglement']; ?>" required>
                            </div>
