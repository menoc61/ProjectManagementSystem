...
$checkSql = "SELECT t.*, p.TitreProjet FROM TACHE t JOIN PROJET p ON t.IdProjet = p.IdProjet WHERE t.IdTache = $id";
...
// Vérifier si la tâche a des affectations associées
$affectationsSql = "SELECT COUNT(*) as count FROM AFFECTATION WHERE IdTache = $id";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['confirm'])) {
    if ($affectationsCount > 0) {
        $error = "Impossible de supprimer cette tâche car elle a des affectations de personnel associées.";
    } else {
        $sql = "DELETE FROM TACHE WHERE IdTache = $id";
        
        if (mysqli_query($conn, $sql)) {
            header("Location: index.php?deleted=1");
            exit();
        } else {
            $error = "Erreur lors de la suppression de la tâche: " . mysqli_error($conn);
        }
    }
}

$tache = mysqli_fetch_assoc($checkResult);
?>
...
<head>
    ...
    <title>Supprimer une Tâche - Gestion de Projets</title>
        ...
        <div class="row">
            ...
            <div class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2">Supprimer une Tâche</h1>
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="mb-4">
                            <p><strong>ID:</strong> <?php echo $tache['IdTache']; ?></p>
                            <p><strong>Libellé:</strong> <?php echo htmlspecialchars($tache['LibelleTache']); ?></p>
                            <p><strong>Projet:</strong> <?php echo htmlspecialchars($tache['TitreProjet']); ?></p>
                            <p><strong>Période:</strong> Du <?php echo date('d/m/Y', strtotime($tache['DateDebutTache'])); ?> au <?php echo date('d/m/Y', strtotime($tache['DateFinTache'])); ?></p>
                        ...
                        <div class="alert alert-warning">
                            <i class="bi bi-exclamation-triangle"></i> Cette tâche a <strong><?php echo $affectationsCount; ?> affectation(s)</strong> de personnel associée(s). La suppression n'est pas possible.
                        </div>
