...
$checkSql = "SELECT * FROM TYPEPROJET WHERE IdTypeProjet = $id";
...
// Vérifier si le type de projet a des projets associés
$projetsSql = "SELECT COUNT(*) as count FROM PROJET WHERE IdTypeProjet = $id";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['confirm'])) {
    if ($projetsCount > 0) {
        $error = "Impossible de supprimer ce type de projet car il a des projets associés.";
    } else {
        $sql = "DELETE FROM TYPEPROJET WHERE IdTypeProjet = $id";
        
        if (mysqli_query($conn, $sql)) {
            header("Location: index.php?deleted=1");
            exit();
        } else {
            $error = "Erreur lors de la suppression du type de projet: " . mysqli_error($conn);
        }
    }
}

$typeProjet = mysqli_fetch_assoc($checkResult);
?>
...
<head>
    ...
    <title>Supprimer un Type de Projet - Gestion de Projets</title>
        ...
        <div class="row">
            ...
            <div class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2">Supprimer un Type de Projet</h1>
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <p class="card-text">
                            Êtes-vous sûr de vouloir supprimer le type de projet suivant ?
                        </p>
                        
                        <div class="mb-4">
                            <p><strong>ID:</strong> <?php echo $typeProjet['IdTypeProjet']; ?></p>
                            <p><strong>Libellé:</strong> <?php echo htmlspecialchars($typeProjet['LibelleTypeProjet']); ?></p>
                            <p><strong>Forfait coût:</strong> <?php echo number_format($typeProjet['ForfaitCoutTypeProjet'], 2, ',', ' '); ?> FCFA</p>
                        </div>
                        
                        <?php if ($projetsCount > 0): ?>
                        <div class="alert alert-warning">
                            <i class="bi bi-exclamation-triangle"></i> Ce type de projet a <strong><?php echo $projetsCount; ?> projet(s)</strong> associé(s). La suppression n'est pas possible.
                        </div>
